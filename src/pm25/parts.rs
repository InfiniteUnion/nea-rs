...
pub struct Pm25Input<S: satay_runtime::StringStorage = String> {
...
impl<S: satay_runtime::StringStorage> Pm25Input<S> {
    pub fn new() -> Self {
        Self {
            date: None,
...
impl<S: satay_runtime::StringStorage> Default for Pm25Input<S> {
    fn default() -> Self {
        Self::new()
    }
}
...
/// - Unit of measure for readings is `µg/m3`.
#[derive(Debug, Clone, PartialEq)]
pub enum Pm25OperationResponse<S: satay_runtime::StringStorage = String> {
    /// PM 2.5 Information
    Ok(Pm25Response<S>),
...
///
/// - Unit of measure for readings is `µg/m3`.
pub fn pm25_parts<S: satay_runtime::StringStorage>(
    input: Pm25Input<S>,
) -> Result<satay_runtime::RequestParts<()>, satay_runtime::Error> {
    let mut uri = String::with_capacity(5);
    uri.push_str("/pm25");
